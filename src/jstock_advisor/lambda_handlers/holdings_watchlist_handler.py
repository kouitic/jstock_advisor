...
def _analyze_holdings(
    ...
    for holding in holdings:
        ...
        if sell_outcome.data_error:
            logger.warning(
                "data_error stock_code=%s error=%s", holding.stock_code, sell_outcome.data_error
            )
            notification_service.notify_data_error(holding.stock_code, sell_outcome.data_error, now)
            continue
        if sell_outcome.recommendation is not None:
            recommended += 1
            recommendation_repo.save(sell_outcome.recommendation)
            if notification_service.notify_recommendation(sell_outcome.recommendation, now):
                notified += 1
            continue
        ...
        if pt_outcome.data_error:
            logger.warning(
                "data_error stock_code=%s error=%s", holding.stock_code, pt_outcome.data_error
            )
            notification_service.notify_data_error(holding.stock_code, pt_outcome.data_error, now)
            continue
        if pt_outcome.recommendation is not None:
            recommended += 1
            recommendation_repo.save(pt_outcome.recommendation)
            if notification_service.notify_recommendation(pt_outcome.recommendation, now):
                notified += 1

    return {"scanned": len(holdings), "recommended": recommended, "notified": notified}
...
def _analyze_watchlist(
    ...
    for item in items:
        ...
        if outcome.data_error:
            logger.warning("data_error stock_code=%s error=%s", item.stock_code, outcome.data_error)
    ...
    return {"scanned": len(items), "recommended": recommended, "notified": notified}
