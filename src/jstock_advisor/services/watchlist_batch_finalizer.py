...
def compute_batch_metrics(records: list[CandidateProgressRecord]) -> dict[str, Any]:
    ...
    rate_limited = sum(1 for r in terminal if r.is_rate_limit_suspected)
    ...
    return {
        ...
        "rate_limit_suspected_count": rate_limited,
        "rate_limit_suspected_rate_pct": (rate_limited / processed * 100) if processed else 0.0,
...
def _finalize_completed(
    ...
    threshold = config.watchlist_screening.high_throttle_rate_threshold_pct

    added_items: list[WatchlistItem] = []
    results_by_code: dict[str, WatchlistScreeningResult] = {}
    if processed > 0 and metrics["rate_limit_suspected_rate_pct"] > threshold:
...
def maybe_finalize(
    ...
    """
    if not try_finalize_if_ready(batch_id):
        return False
    ...
    try:
        _finalize_completed(batch_id, started_at, now, providers, config, notification_service)
