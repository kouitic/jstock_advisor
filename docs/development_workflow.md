...
post-merge / post-deploy の Instruction と、その報告には
`VERIFICATION_REQUIRED = <項目 | NONE>` を必須とする(Issue #220)。書式と適用範囲の
正本は [ai_operation_message_contract.md](ai_operation_message_contract.md) 11節で
あり、本節へ複製しない。
...
更新責務  READ_MODEL_UPDATE_OWNER = ACTOR_WHO_CHANGED_WIP
          WIP を取得・解放した作業者が、同じ作業単位の中でコメントを追記して
          更新する(本文は書き換えない)
```
...
### Definition of Done(DoD) の申告

```
DOD_DECLARATION_REQUIRED = YES
```

PR 本文で、次の 5 項目を **1 行ずつ申告する**。

```
1  境界の連続性
   閾値の直前・直後で結果が不連続に跳ばないこと
2  単調性
   入力が改善したとき、結果が悪化しないこと
3  定常でない 1 回目
   初回・baseline 無し・欠損・非営業日の経路を通したか
4  単位・スケールの整合
   %・倍率・円・株数・営業日の取り違えが無いこと
5  失敗の可視性
   fail-open にしていないこと。失敗が呼び出し側または監視から見えること
```

本節は直近の欠陥を根本原因で束ねた結果である。いずれも
**実装した本人が PR の時点で確認できた**ものであり、レビュワーが気づけるか
どうかに依存させないため、指摘ではなく申告の形にする。

```
型 1  境界の連続性    Issue #186 / #179 / #187(valuation の閾値前後)
型 2  単調性          Issue #260(ばらつきが大きいほど買付価格が高くなる)
型 3  定常でない1回目  Issue #249(baseline 無しの初回評価)
型 4  単位・スケール   Issue #140(単位不整合で変動係数を一度も使わない)
型 5  失敗の可視性     Issue #223(404 で失敗しても cache 継続で検知できない)
```

```
★ 「該当なし」も 1 行で申告する。空欄を許さない。

  空欄だと「確認したうえで該当しない」のか「確認していない」のかを
  読み手が区別できない。区別できなければ、この申告は機能しない。
```

```
★ 満たしていない項目は「該当あり・未解消」とし、
  理由と引き継ぎ先(Issue)を 1 行で書く。

  申告は「すべて満たした」と書くための欄ではない。該当するが今回の PR では
  解消しないなら、そのことを書いて引き継ぎ先を示すほうが、
  「該当なし」と書いて消してしまうより安全である。
  引き継ぎ先を書かせるのは、未解消のまま追跡が切れるのを防ぐためである。

  例  1 境界の連続性 = 該当あり・未解消(前段 band との境界は本 PR の範囲外。#187 へ引き継ぎ)
```

```
★ 申告は自己申告であり、正しさの保証ではない。

  レビュワーは申告と diff が矛盾していないかを見る
  (3.5節の TIME_SEMANTICS_IMPACT と同じ扱い)。
  例: 閾値の定数に diff があるのに「1 = 該当なし」は FAIL とする。
```

記入欄は `.github/PULL_REQUEST_TEMPLATE.md` にある。

```
★ CI では強制しない(未記入でも CI を落とさない)。

  人が読む欄として運用し、機械的な強制の要否は効果と副作用を見てから
  別途判断する。いきなり強制すると、通すためだけの記入が増えて
  申告の意味が失われる。
```
...
```
SAME_TYPE_SWEEP_REQUIRED = YES(根本原因が確定した Issue の close 前)
```

根本原因が確定した Issue は、close の前に**同じパターンが他に無いか**を
1 回だけ掃く。

```
方法  grep / AST 等。**値ではなく形(渡し方・呼び方)で探す**
記録  走査範囲・方法・結果を Issue コメントへ残す(**0 件でも残す**)
扱い  見つかった同型は
        同じ PR で直す、または
        **1 件の Issue へ束ねる**
```
...
★ 3〜5 件に分裂させない。
  ...
  同型は同じ根本原因を持つため、1 件へ束ねたほうが直し方も揃う。
  1 件ずつ起票すると、それ自体が Issue を増やす
  (issue_label_policy.md 7.3.1a と同じ理由)。

★ 0 件でも記録するのは、「調べていない」と「調べて無かった」を
  後から区別できるようにするためである。記録が無ければ区別できない。

★ 値ではなく形で探すのは、値の denylist では**架空値や将来追加される値を
  検出できない**ためである(Issue #131 の pii-scan が denylist 方式であり、
  Issue #135 で AST guard を別に置いたのがこの理由である)。
```

```
★ sweep は「見つけたら全部直す」ではない。

  scope を広げると宣言した lock・レビュー範囲と食い違う(9.5節)。
  見つけたものは**別 Issue へ出す**のが既定であり、
  同じ PR で直すのは「同じファイル・同じ lock の範囲内」に限る。
  実例: Issue #135 の sweep で見つかった 1 件は範囲外だったため
  Issue #256 として別に起票した。
```
