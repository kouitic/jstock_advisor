...
- **買付余力(available cash)の棚卸し(2026-09-26追加、Issue #592)**:
  ...
  仕様書10.6節を参照。CLI版は3.5節)。**リッチメニューへのボタン追加自体は
  本Issueのscope外**(A5b/#593で別途対応)であり、本Issueの時点では
  postback自体は実装済みでも、リッチメニュー上に到達手段が無い
  (postbackを直接送信しない限り、利用者は本機能へ到達できない)。
  #593でボタンが追加されるまで、実質的にShadow実装の状態にある。
