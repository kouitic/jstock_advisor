...
- kill switch(緊急停止)が有効な間は、個別のLINE通知・1日のまとめ通知のいずれも送信されません(判定・Recommendation保存自体は継続します)
- まとめ通知・個別通知(利益保全注意を含む)の重複送信防止は、通常の実行では確実に機能しますが、AWS Lambdaの非同期呼び出しがエラー時に自動再試行される仕様のため、ごく稀に同一銘柄の処理が重複して実行されると二重送信されうる、完全な保証ではないbest-effort方式です(原子的な排他制御への改善はGitHub Issueで追跡)
