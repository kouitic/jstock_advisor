...
- まとめ通知・個別通知(利益保全注意を含む)の重複送信防止は、通常の実行では確実に機能しますが、AWS Lambdaの非同期呼び出しがエラー時に自動再試行される仕様のため、ごく稀に同一銘柄の処理が重複して実行されると二重送信されうる、完全な保証ではないbest-effort方式です(原子的な排他制御への改善はGitHub Issueで追跡)。この重複抑止の「当日」判定は日本時間の暦日基準です(2026-08-21改訂。以前はシステム基盤の時刻(UTC)の暦日をそのまま使っており、日本時間の朝8時台〜9時台でUTC暦日とずれ、同じ日本時間の1日が別日として扱われる不備がありました)
...
6.1のCSVコマンドに加えて、LINEトーク画面下部のリッチメニューから
「📈 買った」「📉 売った」「⭐ お気に入り登録」を選び、画面の案内(入力→
確認→登録する/やり直す/キャンセル)に従って記録する方式にも対応している。
利用者向けの操作方法は機能仕様書10.4節を参照。運用担当者が把握しておくべき
点は以下のとおり。
...
- **既知の制約**: 本方式自体の二重登録防止は上記の原子的操作で保証される
  が、本方式と6.1のCSVコマンド・CLIが「ほぼ同時に同一銘柄」を操作した
  場合の完全な相互排他は対象外(9節参照)。
...
| LINEメニューボタン登録(会話型UI、6.2節) | 6.1と同様、推奨ID・手数料・税額・メモ等は指定不可。本方式自体の二重登録は防止しているが、本方式とCSVコマンド/CLIとの間の「ほぼ同時操作」に対する完全な相互排他は未対応(必要であれば別途PortfolioService全体の楽観ロック化を検討) |
