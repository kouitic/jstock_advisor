...
| S-20 | 観測・Shadow計測の隔離 | `domain/shadow_observation.py` | D1 / D2 / D3 / D4 / D5 / D6 / D9 | `stock_snapshot_service` `holdings_watchlist_handler` `holding_decision_notification_builder`は統合済み。★ `buy_signal_service`(PR-1でisolated_shadow_observation()を抽出したファイル自身。ただし`*_metrics`系8箇所は未隔離のまま残存)・`sell_signal_service`(1箇所未隔離)・`profit_taking_service`(1箇所未隔離)は、PR #401レビュー(サブちゃんFINAL VERDICT)で計10箇所の同型未隔離Shadow整形が判明した。#384は後続対応(USER判断待ち)まで継続 |
