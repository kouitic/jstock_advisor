...
| F-51 | 異常通知の中継・状態管理(#132 X-4。段階1〔CloudWatch Alarm→SNS→LINE〕+ #506でreconciler発のInternal payloadも同じSNS Topic経由で共通normalize + #504〔段階2〕でErrors alarmを全12 Lambdaへ拡大 + #505〔段階2〕でWeeklyReviewFunctionへDuration alarmを追加) | `lambda_handlers/incident_notifier_handler.py` `infrastructure/aws/incident_state_tracker.py` `domain/notification/incident_signal.py` | `incident_notification.yaml` | `IncidentStateTable`(fingerprint単位のclaim/dedup/stale takeover。#508でも再利用予定) | D5 / D9 |
