...
| ID | 機能 | 主要 source | 主要 config | 永続契約 | 影響領域 |
|---|---|---|---|---|---|
| F-26 | 保有・取得ロット管理 | `services/portfolio_service.py` `domain/entities/holding.py` `infrastructure/aws/holding_replacement_commit.py` `infrastructure/local_repository/holding_repository.py` `services/csv_import_service.py` | — | `HoldingsTable` `PurchaseLotsTable` | D6 |
...
| 2026-09-27 | F-01(買い候補日次バッチ)の`lambda_handlers/buy_candidates_handler.py::_record_evaluation_audit()`・`_record_notification_outcome_audit()`で、**評価監査記録・通知結果監査記録を決定的IDのrecord_if_absent()へ統一した**(Issue #531。#71 F-C14。Tier 1のうちPR #622〔watchlist_screening_audit.py側〕からD5競合起因で分離していた残りの部分。サブちゃんによるPR #622レビュー時の指摘で`_record_notification_outcome_audit()`も同型の対象であることが判明したため、HANAKO判断でPR #625のscopeへ追加した)。従来いずれも`AuditService.record()`(呼び出しごとにaudit_idをuuid4で新規生成)のままだったため、非同期fan-out(Lambda再試行)の再配信で監査ログが重複しうる欠陥が#528/#558/#622と同型で残っていた。`_record_evaluation_audit()`へ`batch_id: str | None = None`引数を追加し、非None時は決定的audit_id(`f"unified_buy_candidate_evaluation:{batch_id}:{stock_code}"`)を使う(呼び出し元`_process_single_candidate()`が既に持つIssue #558由来の`batch_id`引数をそのまま渡すのみ。batch_id=None〔白箱テスト等の既存呼び出し〕の場合のみ従来どおりrecord()を維持する後方互換ガード)。`_record_notification_outcome_audit()`は唯一の呼び出し元`_finalize_batch()`が常に非NoneのbatchIdを持つため後方互換分岐は無く、`batch_id: str`(必須)を追加して常に決定的audit_id(`f"unified_buy_candidate_notification_outcome:{batch_id}:{notification_pathway}:{stock_code}"`)を使う(19箇所の呼び出しへ既存のbatch_id変数を渡すのみ)。**サブちゃんによるPR #625自身のblind-firstレビューで、当初`batch_id:{stock_code}`の3要素構成には退行(F1 MUST)があることが判明した**: `_finalize_batch()`内の4つの独立したループ(BUYランキング/送信/NEAR BUYランキング/WATCH終了通知)から呼ばれるため、NEAR_BUY監視中の銘柄が決算接近で当日WATCH_BEFORE_EARNINGSへ切り替わる場合等、同一batch・同一stock_codeが異なる内容で2回呼ばれる経路が実在し、本PR以前(uuid4)は両方残っていたのに対し3要素構成では2件目が黙って抑止されていた。`notification_pathway`(`"buy"`/`"near_buy"`/`"watch_end"`のいずれか)をaudit_idの4番目の構成要素として追加し、mutation-based negative verificationでこの退行が実際に検出されることを確認した(新規テスト)。新規テスト`tests/unit/test_issue_531_buy_candidates_audit_dedup.py`(11件。mutation-based negative verificationと、batch_id/stock_code/notification_pathwayそれぞれを落とすと正当な監査記録が失われることを固定するF2型の粒度テストを含む)で固定した。tests側でAuditServiceをfake/stub化している箇所を再sweepし、`record_if_absent()`未実装だった2件(`test_buy_candidates_handler_dry_run.py`・`test_buy_candidates_handler_latest_batch_pointer.py`)を修正した。#531本文scope外の残存`AuditService.record()`呼び出し(9ファイル・実測18箇所。いずれも呼び出しスコープにbatch_idが無いため対象外と判断)をIssue #531へ記録した(close可否の判断材料)。lock領域はD1/D5/D9(F-01の宣言タグ全体)。**領域一覧・機能一覧・既存のF行(コメント以外)・共通部品カタログは変更していない。** 買い判定・通知内容・保存データ形式(既存フィールドの値のみ)は変更せず、監査記録が重複しなくなる点のみが変わる |
