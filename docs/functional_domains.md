...
| F-48 | 判断の安全条件のshadow計測 | `domain/signals/judgment_safety_shadow_config.py` `domain/signals/judgment_safety.py` `services/judgment_safety_shadow_service.py` | `judgment_safety_shadow.yaml`(専用loader。AppConfigへは載せない) | `AuditLogTable`(`decision_type=judgment_safety_shadow`。PR-3[#457]から。shadowがOFF[既定]なら書かない。新規Table・IAMなし) | D1 / D2 / D3 |
