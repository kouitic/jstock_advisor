...
| F-54 | owner単位の買付余力(available_cash)の永続化・棚卸し更新(#128 A1/A2。Portfolio Capital Allocation Epicの基盤。配分最適化・期待リターン・Capital Rotation・trade連携atomicity・cash増減ロジック・LINE/CLI UIはいずれもA1/A2のscope外で、#128本体・A3以降として別Issueで扱う) | `domain/entities/available_cash.py` `infrastructure/local_repository/available_cash_repository.py` `services/available_cash_service.py` | — | `AvailableCashTable`(新設。owner単位1レコード。schema/repository capability追加のみでinfra配線・Production write/backfillは対象外) | D6 |
