...
| 2026-09-19 | S-21(LINE通知clientの実行時構築)の消費側として`lambda_handlers/watchlist_batch_reconciler_handler.py`(F-16 / D4・D9)を、strict版`build_live_line_client_from_env()`へ切り替えた(Issue #117 Phase B1b-4c)。★ worker/terminal_failureのprescan方式とは異なる: reconcilerは複数の独立した回復処理を担う「最後の安全網」であり、LINE認証情報の欠落(通知と無関係な障害)で、DISPATCHING失敗確定・MAINTENANCE救済・timeout確定・maintenance trigger再試行・buy/holdingsのcompletion recoveryまで止めてはならない。通知サービスを使うのは3か所(RUNNING+NEW_CANDIDATEのmaybe_finalize / retry_finalize / retry_notification)だけなので、その直前で1回だけ構築する遅延getter(`_LazyNotificationService`)を導入した。認証情報欠落(`LineCredentialsMissingError`)はそのバッチの通知が必要な処理だけをskipし(状態不変。RUNNING+NEW_CANDIDATEは救済のみskipしtimeout判定は従来どおり)、全処理の完了後に欠落が1回でもあれば`LineCredentialsMissingError`を送出する(Lambda呼び出しがErrorsとして可視化される)。認証情報欠落以外の構築失敗は握りつぶさず従来どおり伝播する。lock領域はD4/D9(F-16)、対象はreconciler 1 handlerとテスト。`infrastructure/line/client.py`(S-21)は変更していない。**領域一覧・機能一覧・既存のF行・S-01〜S-20は変更していない。** 判定ロジック・通知内容・保存データ形式の変更なし(認証情報が正常な通常運用の挙動は不変) |
