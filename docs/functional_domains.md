...
| 2026-09-26 | F-15(監視候補スクリーニング)の`services/watchlist_screening_audit.py`で、**3種の監査記録(candidate/repository_result/rotation_commit)を決定的IDのrecord_if_absent()へ統一した**(Issue #531。#71 F-C14。Tier 1のうちdomain競合のないwatchlist_screening_audit.py側のみ)。`record_candidate_audit()`/`record_repository_result_audit()`/`record_rotation_commit_audit()`は従来`AuditService.record()`(呼び出しごとにaudit_idをuuid4で新規生成)のままだったため、非同期fan-out(watchlist_worker_handler)・CLI再試行の再配信で監査ログが重複しうる欠陥が#528/#558と同型で残っていた。`record_removal_audit()`は#62 Phase Bで既にrecord_if_absent化済み、`record_batch_audit()`は既存の`idempotency_key`引数経由で既にrecord_if_absent対応済みのため、いずれも本Issueの対象外(変更していない)。決定的audit_idは`f"{decision_type}:{batch_id}:{stock_code}"`(rotation_commitはstock_code無しのため`f"{decision_type}:{batch_id}"`)とし、新規の変換ロジックは作っていない(#528/#558と同じ設計)。`record_candidate_audit()`はbatch_id=None(既存呼び出し)の場合のみ従来どおりrecord()を維持する後方互換ガードを持つ(repository_result/rotation_commitはbatch_idが呼び出し規約上常に非Noneのためガード不要)。新規テスト`tests/unit/test_issue_531_audit_dedup.py`(mutation-based negative verification、audit_idの各構成要素〔decision_type/batch_id/stock_code〕を落とすと正当な監査記録が黒く失われることを固定するF2テストを含む)で固定した。**buy_candidates_handler.py側(F-01。D1/D5/D9)はPR #621(#591)がD5を保持中のため本Issueのscopeには含めず、D5解放後に別PRで追加する**(1 Issue内での実装単位の分離。MANAGER判断)。lock領域はD4/S(F-15の宣言タグ全体)。**領域一覧・機能一覧・既存のF行(コメント以外)・共通部品カタログは変更していない。** 監査記録が重複しなくなる点のみが変わり、判定ロジック・通知内容・保存データ形式(既存フィールドの値のみ)は変更していない |
