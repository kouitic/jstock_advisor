...
| 2026-09-27 | F-01(買い候補日次バッチ)の`lambda_handlers/buy_candidates_handler.py::_record_evaluation_audit()`で、**評価監査記録を決定的IDのrecord_if_absent()へ統一した**(Issue #531。#71 F-C14。Tier 1のうちPR #622〔watchlist_screening_audit.py側〕からD5競合起因で分離していた残りの部分)。従来`AuditService.record()`(呼び出しごとにaudit_idをuuid4で新規生成)のままだったため、非同期fan-out(Lambda再試行)の再配信で監査ログが重複しうる欠陥が#528/#558/#622と同型で残っていた。`_record_evaluation_audit()`へ`batch_id: str | None = None`引数を追加し、非None時は決定的audit_id(`f"unified_buy_candidate_evaluation:{batch_id}:{stock_code}"`)を使う。呼び出し元`_process_single_candidate()`は既にIssue #558由来の`batch_id`引数を持っており、その既存値をそのまま渡すのみ(新しいbatch_id生成ロジックは作っていない)。batch_id=None(白箱テスト等の既存呼び出し)の場合のみ従来どおりrecord()を維持する後方互換ガードを持つ(#622のrecord_candidate_audit()と同型の規約)。新規テスト`tests/unit/test_issue_531_buy_candidates_audit_dedup.py`(5件。mutation-based negative verificationと、batch_id/stock_codeそれぞれを落とすと正当な監査記録が失われることを固定するF2型の粒度テストを含む)で固定した。lock領域はD1/D5/D9(F-01の宣言タグ全体)。**領域一覧・機能一覧・既存のF行(コメント以外)・共通部品カタログは変更していない。** 買い判定・通知内容・保存データ形式(既存フィールドの値のみ)は変更せず、評価監査記録が重複しなくなる点のみが変わる |
