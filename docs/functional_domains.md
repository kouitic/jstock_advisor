...
| F-51 | 異常通知の中継・状態管理(#132 X-4。段階1〔CloudWatch Alarm→SNS→LINE〕+ #506でreconciler発のInternal payloadも同じSNS Topic経由で共通normalize) | `lambda_handlers/incident_notifier_handler.py` `infrastructure/aws/incident_state_tracker.py` `domain/notification/incident_signal.py` | `incident_notification.yaml` | `IncidentStateTable`(fingerprint単位のclaim/dedup/stale takeover。#508でも再利用予定) | D5 / D9 |
