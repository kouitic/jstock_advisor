...
| F-54 | owner単位の買付余力(available_cash)の永続化(#128 A1。Portfolio Capital Allocation Epicの基盤。配分最適化・期待リターン・Capital Rotation・trade連携atomicity・cash増減ロジックはいずれもA1のscope外で、#128本体・A2以降として別Issueで扱う) | `domain/entities/available_cash.py` `infrastructure/local_repository/available_cash_repository.py` | — | `AvailableCashTable`(新設。owner単位1レコード。schema/repository capability追加のみでinfra配線・Production write/backfillは対象外) | D6 |
