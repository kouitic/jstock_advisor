...
| S-20 | 観測・Shadow計測の隔離 | `domain/shadow_observation.py` | D1 / D2 / D3 / D4 / D5 / D6 / D9 | `buy_signal_service` `sell_signal_service` `profit_taking_service` `stock_snapshot_service` `holdings_watchlist_handler` `holding_decision_notification_builder`(すべて統合済み。#384 PR-1〜5完了。`src/`全体を`Shadow計測`/`DecisionSnapshot記録専用`でgrepした結果、残る一致は算出関数自身[`domain/signals/*.py`・`domain/entities/*.py`]とconfig/分類ロジックのコメントのみで、未隔離のinline呼び出しは無いことを確認した) |
