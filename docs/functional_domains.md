...
| F-07 | 利確判定 | `domain/signals/profit_taking.py` `services/profit_taking_service.py` `domain/classification/profit_taking_industry.py` `domain/signals/trading_unit_feasibility.py` | `profit_taking_rules.yaml` | `RecommendationsTable` | D2 / S |
| F-08 | 下落保護 | `domain/signals/profit_protection.py` | `sell_rules.yaml` | `RecommendationsTable` | D2 |
| F-09 | 売値レンジ算出 | `domain/signals/exit_price_range.py` `services/sell_price_recommendation_service.py` `domain/entities/exit_price_range.py` | `entry_exit_price_rules.yaml` | `ExitPriceRange`(Recommendation 内) | D2 / S |
...
| F-20 | 監視結果の表示・要約整形 | `services/watchlist_view_service.py` `services/watchlist_judgment_summary_formatter.py` `services/watchlist_score_detail.py` `services/watchlist_addition_summary_builder.py` `domain/ranking.py` `infrastructure/local_repository/stock_name_override_repository.py` `services/watchlist_display_name.py` | — | なし | D4 / D5 |
...
| ID | 機能 | 主要 source | 主要 config | 永続契約 | 影響領域 |
|---|---|---|---|---|---|
| F-26 | 保有・取得ロット管理 | `services/portfolio_service.py` `domain/entities/holding.py` `infrastructure/aws/holding_replacement_commit.py` `infrastructure/local_repository/holding_repository.py` `services/csv_import_service.py` `services/trade_registration_service.py`(Issue #619。CLI経由のBUY/SELL登録をHolding/Transaction/AvailableCashと整合させるorchestration層) | — | `HoldingsTable` `PurchaseLotsTable` | D6 |
...
| ID | 機能 | 主要 source | 主要 config | 永続契約 | 影響領域 |
|---|---|---|---|---|---|
| F-31 | 週次・月次・四半期レビュー | `lambda_handlers/weekly_review_handler.py` `lambda_handlers/monthly_review_handler.py` `lambda_handlers/quarterly_review_handler.py` `services/review_report_service.py` `services/weekly_improvement_review_service.py` `infrastructure/local_repository/weekly_review_metrics_repository.py` `domain/entities/weekly_evaluation_aggregate.py` `services/weekly_evaluation_aggregate_service.py` `infrastructure/weekly_evaluation_aggregate_store.py` `infrastructure/aws/weekly_evaluation_aggregate_dynamodb.py` `cli/weekly_aggregate.py` | `review_improvement.yaml` `schedule.yaml` | 週次レビュー指標の保存先 / `WeeklyEvaluationAggregateTable`(週次評価集計。Issue #537。読み取りは環境変数`WEEKLY_AGGREGATE_READ_ENABLED`で有効化。既定は無効) | D7 / D5 |
| F-32 | 判定の事後評価 | `lambda_handlers/evaluation_handler.py` `services/recommendation_evaluation_service.py` `services/decision_performance_service.py` `domain/entities/evaluation.py` `domain/evaluation_rules.py` `infrastructure/local_repository/evaluation_repository.py` `services/performance_metrics_service.py` | `evaluation_rules.yaml` `decision_evaluation.yaml` | `EvaluationResultsTable` / `WeeklyEvaluationAggregateTable`(暦日7日の評価の保存時に、同じ Transaction で加算。Issue #537。書き込みは環境変数`WEEKLY_AGGREGATE_WRITE_ENABLED`で有効化。既定は無効) | D7 |
| F-33 | 較正・バックテスト | `services/calibration_analysis_service.py` `services/calibration_dataset_service.py` `services/backtest_service.py` `services/holding_decision_backtest_service.py` `services/before_after_report_service.py` `services/holding_decision_compare_service.py` `analysis/valuation_shadow_analysis.py` `analysis/valuation_shadow_hypotheses.py` | — | なし(読み取り中心) | D7 |
...
| F-36 | 財務・配当データ取得 | `providers/financial_data/` `providers/dividend_data/` `domain/financial_series.py` `domain/financial_decomposition.py` `domain/signals/dividend_cut_analysis.py` | — | なし | D8 / S |
...
| F-49 | 異常通知の本文組み立て(#132 X-2。純粋な関数と型。送信・接続・永続化は含まない + #349でSQS DLQのqueue名→job名解決を追加) | `domain/notification/incident_message.py` | — | なし | D5 |
| F-50 | 異常通知の fingerprint / dedup 判定(#132 X-3。純粋な関数と型。永続化・接続は含まない) | `domain/notification/incident_fingerprint.py` | — | なし | D5 |
| F-51 | 異常通知の中継・状態管理(#132 X-4。段階1〔CloudWatch Alarm→SNS→LINE〕+ #506でreconciler発のInternal payloadも同じSNS Topic経由で共通normalize + #504〔段階2〕でErrors alarmを全12 Lambdaへ拡大 + #505〔段階2〕でWeeklyReviewFunctionへDuration alarmを追加 + #349でDLQ滞留alarm〔SQSベース〕のQueueName dimension解決を追加 + #508でGitHub Issue自動起票を追加) | `lambda_handlers/incident_notifier_handler.py` `infrastructure/aws/incident_state_tracker.py` `domain/notification/incident_signal.py` `domain/notification/incident_github_issue_message.py` `services/incident_github_issue_service.py` | `incident_notification.yaml` | `IncidentStateTable`(fingerprint単位のclaim/dedup/stale takeover。#508でgithub_issue_number/github_issue_create_statusを実装) | D5 / D9 |
...
| F-54 | owner単位の買付余力(available_cash)の永続化・棚卸し更新 + CLIからの参照・棚卸し更新(#128 A1/A2/A5c。Portfolio Capital Allocation Epicの基盤。配分最適化・期待リターン・Capital Rotation・trade連携atomicity・cash増減ロジック・LINE UI・rich menuはいずれもscope外で、#128本体・A3/A4/A6以降として別Issueで扱う) | `domain/entities/available_cash.py` `infrastructure/local_repository/available_cash_repository.py` `services/available_cash_service.py` `cli/available_cash.py` | — | `AvailableCashTable`(新設。owner単位1レコード。schema/repository capability追加のみでinfra配線・Production write/backfillは対象外) | D6 |
...
| S-07 | 銘柄・業種分類 | `domain/classification/` `config/stock_classification_rules.yaml` `config/industry_scoring_policy.yaml` `domain/entities/classification.py` | D1 / D2 / D3 / D4 | 買い / 利確 / 財務 / 正規化の各分類 |
...
| S-11 | 決算イベント | `domain/signals/earnings_surprise.py` `domain/signals/earnings_trend.py` `domain/signals/earnings_window.py` `domain/entities/earnings_surprise.py` `domain/entities/earnings_trend.py` | D1 / D2 / D3 | 各判定 |
| S-12 | 市場・セクター環境 | `domain/signals/market_environment.py` `domain/signals/sector_environment.py` `domain/signals/_environment_shared.py` `domain/entities/environment.py` `domain/entities/market_environment.py` `domain/entities/sector_environment.py` `domain/signals/environment.py` | D1 / D2 / D4 | 各判定 |
...
| 2026-09-21 | F-07(利確判定)で、**上限価格(想定上限価格)が使えない原因を、原因ごとの理由コードで利用者・監査へ伝えるようにした**(Issue #471。USER決定 U-a〜U-d)。`domain/entities/valuation.py`(共通部品 S-05)の`ProfitTakingFairValueBlockReasonCode`へ4値を追加した(`TOO_FEW_METHODS_FOR_ACTION` / `FAIR_VALUE_NOT_REFLECTING_LATEST_EARNINGS` / `FAIR_VALUE_EARNINGS_REFLECTION_UNKNOWN` / `EARNINGS_TOO_CLOSE_FOR_ACTION`。既存の値は不変。**追加のみ**)。`domain/signals/profit_taking.py`の理由の導出は、`_fair_value_action_usable()`と**同じ入力・同じ比較**で行い(判定の真偽は変更していない。全組合せで矛盾しないことをテストで固定)、複数原因は評価順を**定数で固定**して全原因を保持する(`ProfitTakingResult.fair_value_action_block_reason_codes`。末尾の任意field。既定は空)。利用者表示は最初の1つ(`fair_value_action_block_reason_code`。従来どおり)、**監査は全原因**(`profit_taking`の監査記録の`output_values`へ`fair_value_action_block_reason_codes`を追加。dictの値のため永続schemaは不変)。`services/profit_taking_service.py`の文言は USER確定の原文どおりで、閾値はconfigの実値。**既存の「次回決算まで N 営業日」の行は、EARNINGS_TOO_CLOSE_FOR_ACTIONの文言へ置き換えた**(U-b = REPLACE。決算直前の閾値は、遮断側`min_business_days_to_earnings_for_fair_value_action`の1か所へ一本化)。`services/stock_analysis_view_service.py`の固定文言辞書へ4値を追加した(未知のコードは従来どおり汎用文言へ落ちる。rollbackで新旧が混在しても崩れない)。**Recommendation・永続schema・config・IAM・共通enum S-16は変更していない**。対象外(U-a): 業種区分・bull/bear欠如・レンジ無し・含み損。別Issue(U-d)= #509(財務鮮度の定義の2重化)。既存のF行・S行・領域一覧・維持契約は変更していない |
...
| 2026-09-24 | F-51(異常通知の中継・状態管理)の監視対象を、EvaluationFunction 1関数からLambda **12関数全て**へ拡大した(Issue #504〔#132 X-5。段階2〕)。既存2 alarm(Evaluation Errors/Duration)は#503で先行済みのため変更せず、残る11関数(BuyCandidates/HoldingsWatchlist/DisclosureCheck/WatchlistDispatcher/WatchlistWorker/WatchlistTerminalFailureHandler/WatchlistBatchReconciler/WeeklyReview/MonthlyReview/QuarterlyReview/LineWebhook)へ、既存EvaluationFunctionErrorsAlarmと同一設計(Statistic=Sum・Period=900・EvaluationPeriods=1・Threshold=1・ComparisonOperator=GreaterThanOrEqualToThreshold・TreatMissingData=notBreaching)のErrors alarmを新設し、AlarmActionsを`IncidentNotificationTopic`(#503)へ統一した。**既知の限界**: per-item/per-batchの例外を`except Exception`で広く捕捉しre-raiseしない設計の6関数(BuyCandidates/HoldingsWatchlist/WatchlistDispatcher/WatchlistWorker/WatchlistBatchReconciler/LineWebhook)では、この種の内部異常はLambdaのErrorsメトリクスに計上されないため、本Issueのalarmだけでは検知できない(終端状態の監視は#506/#507が担当)。残る6関数(DisclosureCheck/Evaluation/WeeklyReview/MonthlyReview/QuarterlyReview/WatchlistTerminalFailureHandler)は、ハンドラ本体がper-item/per-batchの例外を握り潰さないため上記6関数より検知範囲が広い。**ただし「Errorsだけで完全に検知できる関数」は無い**: この6関数であっても、ハンドラが呼ぶ個別module内に独自の`except Exception`があれば同様に計上されない(実例: `evaluation_run_audit.py`の監査書き込み失敗はflagに依らず常に`except Exception`で握り潰され、Evaluationはこの一点を検知できない)。Duration alarm(#505)・Production deploy(別Human Gate)は本Issueのscope外。既存の判定・通知内容・保存形式・領域一覧は変更していない |
| 2026-09-24 | F-51(異常通知の中継・状態管理)へ、**WeeklyReviewFunctionのDuration alarm**を追加した(Issue #505〔#132 X-6。段階2〕)。既存の`EvaluationFunctionDurationAlarm`(#113/#503)と同一設計(Statistic=Maximum・Period=900・EvaluationPeriods=1・ComparisonOperator=GreaterThanOrEqualToThreshold・TreatMissingData=notBreaching)で、`WeeklyReviewFunctionDurationAlarm`をThreshold=240,000ms(Timeout 300秒 × 0.8)で新設し、AlarmActionsを`IncidentNotificationTopic`(#503)へ接続した。240秒という値は、Issue #377(PR #539。週次改善レビューのOOM修正)後もWeeklyReviewFunctionのTimeout自体が300秒のまま変わっていないことを確認したうえでの計算根拠であり、#539後の実際のDuration実測値による再検証は次回自然実行(2026-09-28 19:00 JST)後に持ち越す(人工的な実行はしない)。他10関数(WatchlistWorker/WatchlistDispatcher/LineWebhook/BuyCandidates/HoldingsWatchlist/DisclosureCheck/WatchlistBatchReconciler/MonthlyReview/QuarterlyReview/WatchlistTerminalFailureHandler)へのDuration alarm追加は今回見送り(MANAGER判断)。既存の判定・通知内容・保存形式・領域一覧は変更していない |
...
| 2026-09-24 | F-49(異常通知の本文組み立て)・F-51(異常通知の中継・状態管理)へ、**DLQ滞留のCloudWatch Alarm 4本**を追加した(Issue #349。#132 X-7相当の恒久監視。#117 D2で残された3点の恒久監視要求の1点目)。対象4本(WatchlistTerminalFailureDLQ/AsyncInvokeFailureDLQ/BuyCandidateTerminalFailureDLQ/HoldingsWatchlistTerminalFailureDLQ)いずれも、SQSのApproximateNumberOfMessagesVisible>=1(Statistic=Maximum・Period=300・EvaluationPeriods=1・ComparisonOperator=GreaterThanOrEqualToThreshold・TreatMissingData=notBreaching)で即時incident化し、既存の`IncidentNotificationTopic`(#503)へ接続した(新規Topic・Topic Policyは追加していない)。DLQは自動消費者が無く再試行を使い切った終端失敗が既に確定した事実であるため、#507のS-6(処理遅延の持続判定)とは異なり即時検知とした。USER決定により、dormant(#319 Phase 1。Phase 2でdispatch側が切り替わるまでコードから未参照)な2本(BuyCandidate/HoldingsWatchlist)も、将来の監視漏れ防止のため先行して追加した。`domain/notification/incident_message.py`の`IncidentJob`へ新規member`ASYNC_INVOKE_FAILURE`(「非同期実行の失敗」)を追加し、AsyncInvokeFailureDLQ(BuyCandidatesFunction/HoldingsWatchlistFunctionの両方が共有し、メッセージ単体からはどちらの関数由来か区別できない)を既存の買い候補チェック/保有株チェックのいずれか一方へ誤って割り当てないようにした(USER決定)。同モジュールへSQS DLQのqueue名→jobの対応表(`_QUEUE_NAME_TO_JOB`)を新設し、既存のLambda関数名の対応表(`_INTERNAL_NAME_TO_JOB`。全12+1関数を網羅する既存の完全一致テストの対象)とは別に保持した(混在させると既存の網羅性テストが赤くなるため)。`lambda_handlers/incident_notifier_handler.py`の`_extract_function_name()`を`_extract_alarm_target()`へ改称し、AlarmのSNS payloadで`FunctionName` dimensionが見つからない場合に`QueueName` dimensionへfallbackするよう拡張した(既存のFunctionName dimension経路の挙動・優先順位は変更しない)。`docs/operations_manual.md` 24節(DLQの滞留の確認手順)へ、Alarmが接続されたことの反映と、「Alarm→OKへ戻っても対象job・batchが復旧したことを意味しない」ことの明記(redrive/purge/retention経過のいずれでもQueue depthは0へ戻るため)を追記した。Production deployは別Human Gate(本PRは実施しない)。既存の判定・通知内容・保存形式・領域一覧は変更していない |
| 2026-09-24 | F-51(異常通知の中継・状態管理)へ、**incidentのGitHub Issue自動起票**を追加した(Issue #508。#132 X-9。PR-1、D5範囲のコードのみ。USER決定U-1〜U-5)。`infrastructure/aws/incident_state_tracker.py`に、#503が予約していた`github_issue_number`/`github_issue_create_status`を実装した(LINEのclaim/dedup状態機械`status`〔CLAIMED/SENT〕とは完全に独立した別の状態機械。`improvement_task_tracker.py`〔週次改善レビュー〕と同じ「claim → stale時はGitHub側の実在確認 → 明示的な再claim」の2段階方式を踏襲)。新規`domain/notification/incident_github_issue_message.py`(GitHub Issue本文/タイトルの純粋なbuilder。#501 `incident_message.py`と同じallowlist契約)・新規`services/incident_github_issue_service.py`(既存のGitHub App資産`infrastructure/github/client.py`〔F-34。週次改善レビュー機能で稼働中〕を再利用したIssue作成・コメント追記のオーケストレーション。OPEN→コメント追記/CLOSED→新規Issue〔reopenしない〕/stale claim reconciliation)を追加した。`config/incident_notification.yaml`へ`issue_creation_enabled`(既定false)・`github_issue_claim_timeout_minutes`・`issue_labels`を追加した。`lambda_handlers/incident_notifier_handler.py`はGitHub Issue処理をLINE通知経路から独立したtry/exceptで呼び出す(GitHub側の失敗がLINE経路に一切影響しないことをhandlerレベルのテストで固定)。**実装時に検知した設計上の注意点**: 初出incidentでLINE送信が失敗すると既存契約(#503)の`release_claim(is_new=True)`がfingerprint行をDeleteItemするため、この場合に限りGitHub側の試行を次のSNS/Lambda retryへ委ねる(row削除直後の書き込みによるLINE側再claimの永久破壊を回避)。**本PRはinfra/template.yamlを変更していない**(`GITHUB_APP_SECRET_ARN`/`GITHUB_REPOSITORY`環境変数・`secretsmanager:GetSecretValue` IAMの配線はD9 domain lockの都合で別PRとする)ため、`issue_creation_enabled`が既定falseであることと合わせ、本PRのmerge単独ではProductionの挙動・GitHub API呼び出しは一切発生しない。既存の判定・通知内容・保存形式・領域一覧は変更していない |
| 2026-09-25 | F-13(取引停止・クールダウン)へ、**売買検知の部分適用クラッシュ時のtakeover再検知漏れを解消するconsumption step**を追加した(Issue #529。#71 F-C11 Phase 2。JIRO Phase A調査+USER/MANAGER判断)。Phase 1(#527)は検知した売買イベントを`TradeEventRecord`(`pending_marker=PENDING`)として永続化するが、`trade_detection_lock`のCOMPLETED遷移が`WatchStateService.end_for_trade_events()`実行より**先**に発生するため、両者の間でLambdaがクラッシュするとロックはCOMPLETEDのまま固定され、以後のいかなる`detect_and_apply()`呼び出しも`events=[]`を返し、WatchState終了が永久に行われないギャップがあった(JIRO実測。ロック自体のstale-lock takeoverは現行の呼び出し元構成〔両handlerとも同一cron一発起動〕では実際には発火しない)。専用batch/新規scheduled Lambdaは作らず、既存の毎時`watchlist_batch_reconciler_handler.py`(F-16)の`handler()`冒頭へ、既存のwatchlist batch reconciliationループとは独立したtry/except境界で新規`services/trade_event_reconciliation_service.py::reconcile_pending_trade_events()`を呼ぶ形で相乗りした(USER/MANAGER判断: 既存GSI`pending-marker-index`・`query_by_index()`が既に実装済みのため新規インフラ不要)。`infrastructure/local_repository/trade_event_record_repository.py`へ`list_pending_with_raw()`(GSI経由の未消費イベント一覧取得)・`mark_consumed()`(`replace_if_raw_matches()`によるCAS。DynamoDBの`put_item`はアイテム全体を置換するため、新しいItemに`pending_marker`を含めないだけでsparse GSIから自動的に外れる)を追加した。一貫性モデルはat-least-once-with-idempotent-consumer(`WatchState._end()`のCAS+終了済みno-op・`TradeEventRecord`の決定的event_idにより、複数reconcilerの並行実行や再実行があっても最終的に1回分の消費へ収束する。2回目以降のCAS不成立はエラー扱いにしない)。`config/notification_rules.yaml`へ`trade_event_reconciliation.max_records_per_run`(1回のreconciler実行あたりの処理上限件数。無制限に長時間化させないための境界)を追加した。`domain/signals/trade_event_detection.py`(`TradeEvent`)・`services/watch_state_service.py`(F-17)はいずれも無変更(`TradeEventRecord`から`TradeEvent`を1:1で再構成して呼び出すのみ)。lock領域はD1/D2/D3(F-13)+D4/D9(F-16、呼び出し配線のみ)。**領域一覧・機能一覧・既存のF行(F-13のsource list以外)・共通部品カタログは変更していない。** 既存の判定・通知内容・保存データ形式(既存collection)の変更なし(新規consumption stepの追加のみ) |
| 2026-09-25 | F-10(保有継続判断)の`BaselineValueSnapshot`(`domain/entities/holding_decision.py`)のdocstring/field commentを実態へ合わせて是正した(Issue #469。TARO Phase A調査+USER決定〔案(ii)。dead field削除は今回承認していない〕)。同構造体は「投資ストーリー維持スコアの標準5軸のbaseline比較対象」を意図した設計だったが、実際にbaseline比較へ使われるのは`has_shareholder_benefit`(優待条件。#470/#476)のみで、`total_yield_pct`/`equity_ratio_pct`は生成時に値を入れるが読み出されない書き込み専用field、`dividend_policy_note`/`benefit_condition_note`/`operating_income_trend_note`/`operating_cashflow_trend_note`の4項目は生成時から一度も値を設定されない(TARO実測: grep 0件)。配当方針・総合利回りは絶対条件でbaseline比較が不要という設計そのものが正しく、利益CF前提・財務健全性は本構造体を経由しない**別の既存機構**(`sell_rule_inputs.evaluations`のトレンド判定・`financial_health_severe_deterioration`のトリガー評価)が実際にbaseline/トレンド比較の役割を果たしている(「baseline_valuesを経由しない」ことと「baseline比較をしていない」ことは同じではない)。`services/holding_decision_service.py`の生成箇所へも同じ契約のコメントを追加した。**コード側のsemantics・永続schema・判定ロジックは一切変更していない**(docstring/comment/testのみ)。dead field削除・serialization/historical persistence互換性への変更は今回のscope外(別Issue化を検討する場合はUSER判断が必要)。lock領域はD3のみ(LOCK_LEVEL 1、コメント/docstring以外は変更しないため)。既存の判定・通知内容・保存データ形式・領域一覧は変更していない |
| 2026-09-25 | 機能一覧へ**新規F-54**(owner単位のavailable_cash永続化)を追加した(Issue #584。#128〔Portfolio Capital Allocation Epic〕の子Issue分解A1。USER決定でPhase B承認、#128本体〔配分最適化・期待リターン・Capital Rotation〕は引き続き未承認)。新規`domain/entities/available_cash.py`(`AvailableCash`。owner単位1レコード、`available_cash>=0`のvalidator、`updated_at`/`last_reconciled_at`のtimezone-aware必須validator)・新規`infrastructure/local_repository/available_cash_repository.py`(`AvailableCashRepository`。既存の`holding_decision_runtime_config_repository.py`と同型の設計〔`insert_if_absent()`による初回作成 + `replace_if_raw_matches()`による楽観ロック更新〕を踏襲。新規CAS方式は作っていない)を追加した。`domain/entities/enums.py`へ`AvailableCashUpdateType`(TRADE_UPDATE/USER_RECONCILIATION)を追加した。**trade登録時のcash増減・atomicity・insufficient cash拒否・LINE/CLI・Expected Return/Portfolio optimizer/Capital Rotation等はいずれもA1のscope外**(#584本文どおり。A2以降として別Issueで扱う)。migration/backfillは実施していない(既存ユーザーへの`available_cash=0`等の自動backfillは行わず、schema/repository capability追加のみ)。infra/template.yamlは変更していない(AvailableCashTableの新設・IAM配線は別Human Gate)。既存のF行・S行・領域一覧・維持契約は変更していない。既存の判定・通知内容・保存データ形式(既存collection)・Production挙動の変更なし |
| 2026-09-25 | F-01(買い候補日次バッチ)・F-46(保有監視日次バッチ)の**EventBridge Schedulerのretryによるbatch二重dispatchを防止した**(Issue #558。Phase A調査+USER/MANAGER判断)。`buy_candidates_handler.py`/`holdings_watchlist_handler.py`のtop-level batch_idはretryのたびに`uuid4()`で再生成されており、かつ`infrastructure/aws/batch_tracker.py::start_batch()`がConditionExpression無しの`put_item()`だったため(#65 F-E7のdispatcherと異なり、既存leaseが一切無かった)、Scheduler retryは新しいbatch_idで全対象を再度fanoutしうる状態だった(Issue #318当時のtemplate.yamlコメントで既に指摘されていた既知の未対応事項)。`derive_scheduled_batch_id()`(#65 F-E7が導入した`_derive_batch_id()`を、3ハンドラで共有するため`lambda_handlers/_scheduling.py`〔F-42〕の共有関数へ昇格。`watchlist_dispatcher_handler.py`側もこちらを呼ぶよう置き換え、private実装は削除)でbatch_idをEventBridge Schedulerの`scheduled_time`から決定論的に導出し、`start_batch()`を`ConditionExpression="attribute_not_exists(batch_id)"`付きのatomicなput_itemへ変更した(戻り値を`None`から`bool`へ変更。`True`=開始権取得、`False`=同一batch_idが既に存在〔正常なidempotency結果。ERRORにしない〕。呼び出し元2箇所〔buy/holdings〕は従来いずれも戻り値を参照していなかったため後方互換)。両handlerとも`start_batch()`が`False`を返した場合はfanout・通知・進捗初期化のいずれも行わずに終了する。`infra/template.yaml`の`BuyCandidatesFunction`/`HoldingsWatchlistFunction`の`ScheduleV2`へ、dispatcherと同じ`Input: '{"scheduled_time": "<aws.scheduler.scheduled-time>"}'`を追加した。子Lambdaへのfanout自体の再試行による個別銘柄の二重評価(Issue #71 F-D2)は本Issueのscope外(据え置き)。lock領域はD1/D2/D3/D4/D5/D9/S(F-46の宣言タグ全体。F-01/F-16/F-42/F-43はいずれもこの部分集合)。**領域一覧・機能一覧・既存のF行・共通部品カタログは変更していない。** 既存の判定・通知内容・保存データ形式の変更なし(通常のScheduler起動〔retryではない初回呼び出し〕の挙動は不変) |
| 2026-09-26 | F-54(owner単位の買付余力の永続化)へ、**ユーザーによる棚卸し更新(reconciliation)機能**を追加した(Issue #589。#128〔Portfolio Capital Allocation Epic〕A2。#584/A1のfollow-up)。新規`services/available_cash_service.py`(`AvailableCashService`)を追加し、`get(raw_owner)`(owner単位の現在値取得。未登録ownerと0円を区別する既存契約〔#584 T4〕を維持)・`reconcile(raw_owner, new_amount, now)`(ユーザー入力値による絶対値上書き。過去理論値との差額イベントは作らない)を実装した。owner正規化は既存`domain/entities/owner.py::normalize_and_validate_owner()`を再利用し(#584 PR #587レビュー指摘AC1への対応。新規の正規化ロジックは作っていない)、初回作成は既存の`AvailableCashRepository.initialize()`(insert_if_absent)、既存owner更新は既存の`get_raw()`+`replace_if_raw_matches()`(CAS)を再利用した。stale/concurrent update対策として、`services/investment_thesis_service.py::activate_baseline()`と同型のbounded retryループ(既定3回、`AvailableCashReconciliationExhaustedError`で打ち切りを可視化)を実装した(新しいCAS方式・独自の排他制御は作っていない)。USER_RECONCILIATIONは前回値に依存しない絶対値上書きであるため、retry時に前回状態を再計算するロジックは不要である。**trade連携によるcash増減(TRADE_UPDATE)・LINE/CLI UI・insufficient cash guard・DynamoDB table作成・IAM・migration/backfillはいずれもA2のscope外**(#589本文どおり。A3以降として別Issueで扱う)。lock領域はD6のみ(F-54)。`domain/entities/owner.py`(S-18)は呼び出しのみで変更していない(LOCK_LEVEL 1)。**領域一覧・機能一覧・既存のF行(F-54のsource list以外)・共通部品カタログは変更していない。** 既存の判定・通知内容・保存データ形式(既存collection)・Production挙動の変更なし |
| 2026-09-26 | F-14(保有スナップショット。`services/stock_snapshot_service.py`)の`build_stock_snapshot()`内、**価格履歴・52週安値・過去株価レンジの基準日をUTC暦日(`now.date()`)からJST暦日(`evaluation_date_jst(now)`)へ統一した**(Issue #475。#472 Phase Aの実測に基づく設計をTAROが完了済み、JIROが実装)。決算関連の暦日比較は既に`evaluation_date_jst(now)`(JST暦日)を使っていたが、価格窓側は`now.date()`(UTC暦日)のままだったため、同じsnapshot内で決算はJST・価格窓はUTCという基準日の不整合があった(JST 00:00-08:59台の実行では前日UTC日付になり、窓の始端が本来より1日広くなる。#472 Phase A固定clock実測: 影響は窓最古の1本のみで頻度は低い見込み)。`history_start`/`get_price_history()`のend/TOPIX・セクターETFのbenchmark historyのend/`compute_historical_range_price()`のas_of_date/`compute_52_week_low()`のas_of_dateの計5呼び出し箇所すべてを、既存の`evaluation_date`(=`evaluation_date_jst(now)`。決算判定と同じ変数)へ統一した。`domain/valuation/fair_value.py`の`compute_52_week_low()`/`compute_historical_range_price()`自体は変更していない(呼び出し側が渡す基準日のみ)。新規テスト(`test_issue_475_stock_snapshot_jst_date_window.py`)は、これら5箇所へ実際に渡された引数を`unittest.mock.patch`でspy/recordして直接検証する形とした(downstream の5手法集計・outlier filterを経由すると検証対象が複雑になるため)。lock領域はD1〜D3(Issue本文の見込みどおり)+SHARED S-05(バリュエーション。S-05自体のコード・契約は変更していない)。**領域一覧・機能一覧・既存のF行・共通部品カタログは変更していない。** 判定ロジックの基準日は変更したが、通知内容・保存データ形式は変更していない(価格窓の境界日〔365日前ちょうど〕に該当する銘柄でのみ、52週安値・過去株価レンジ方式の算出値が変わりうる。既存関連テスト723件で回帰なしを確認済み) |
| 2026-09-26 | F-54(owner単位の買付余力)へ、**CLIからの参照・棚卸し更新コマンド**を追加した(Issue #594。#128〔Portfolio Capital Allocation Epic〕A5c。#589/A2のfollow-up)。新規`cli/available_cash.py`を追加し、`show --owner`(既存`AvailableCashService.get()`をそのまま呼ぶ薄いCLI層。未登録ownerと0円を表示上も明示的に区別する既存契約〔#584 T4〕を維持)・`reconcile --owner --amount`(既存`AvailableCashService.reconcile()`をそのまま呼ぶ。負値は既存entityのvalidatorが送出する`ValueError`をそのままユーザーへ表示してexit code 1とする)を実装した。`cli/main.py`へ`app.add_typer(available_cash.app, name="available-cash")`を追加した。amount引数の数値変換は既存`infrastructure/external_value_parser.py::ExternalValueParser.decimal()`を`cli/holdings.py`の`_parse_decimal()`と同型のfile-local helperで呼ぶのみで、新規の変換ロジックは作っていない。**CLI層自体はowner正規化・CAS・retry・validationのいずれも実装せず、既存`AvailableCashService`へ委譲するのみ**(新しい業務ロジックの追加なし)。LINE/rich menu・trade連携・insufficient cash guard・DynamoDB table作成・IAM・migration/backfillはいずれも本Issueのscope外(#594本文どおり。A3/A4/A6以降として別Issueで扱う)。lock領域はD6のみ(F-54)。**領域一覧・機能一覧・既存のF行(F-54のsource list以外)・共通部品カタログは変更していない。** 既存の判定・通知内容・保存データ形式(既存collection)・Production挙動の変更なし |
| 2026-09-26 | F-11(投資仮説の管理と採点)へ、**register_condition() / attest_condition()のlost update対策と、get_or_create_thesis()の並行create重複防止**を追加した(Issue #570)。#530と同型のCAS(`services/write_plan.py::apply_conditional_put()`。既存のCollectionStore CAS primitive〔#17〕をそのまま使う)を`infrastructure/local_repository/investment_thesis_repository.py`へ追加(`get_raw_data`/`insert_if_absent`/`replace_if_raw_matches`/`delete_if_raw_matches`)し、read直後・updated構築より前にexpected_dataを取得する(#530と同じ理由。構築後に取り直すと並行更新をCASが素通りする)。競合時は`ConcurrentUpdateError`(ValueErrorサブクラス)を送出し**自動リトライしない**(`activate_baseline()`のbounded-retryパターンとは異なる。register_condition/attest_conditionは呼び出し元が最新状態を読み直して再試行する設計とした。design issuecomment-5841369172で確定済み)。`get_or_create_thesis()`は`investment_thesis_id`をholding_id自体から決定的に導出し、既存の`insert_if_absent()`で原子的に作成することで、同一holding_idへの並行create競合を1件へ収束させる(新しいlock/lease機構は作らない)。旧形式(uuid4のinvestment_thesis_id)の既存レコードは`get_by_holding()`の線形scanで引き続き見つかる(migration不要)。`cli/holding_decision.py::register_thesis_condition`へ`ConcurrentUpdateError`(ValueErrorサブクラス)捕捉の`try/except`を追加した(`attest_thesis_condition`の既存パターンと同型)。**register_condition()のVALIDATION実行時の非対称(is_validationでもsave()を無条件に呼ぶ既存挙動)は変更していない**(OUT_OF_SCOPE。是正を検討する場合は別Issue化する)。lock領域はD3のみ(SHARED `services/write_plan.py`は呼び出しのみで変更していない)。**領域一覧・機能一覧・既存のF行(F-11のsource list以外)・共通部品カタログは変更していない。** 判定ロジック・通知内容・保存データ形式(既存collection)の変更なし。Production挙動は、並行更新が実際に発生した場合にのみ変わる(従来はlost updateが無言で発生していたが、以後はCLIがエラーで停止する) |
| 2026-09-26 | F-39(銘柄ユニバース収集)の`providers/candidate_universe/jpx_impl.py`で、**JPXキャッシュのstaleness判定基準日をUTC暦日からJST暦日へ統一した**(Issue #578。#66 16C)。`_check_staleness()`/`get_candidate_universe()`の`cache_age_hours`算出が、JST公表日である`source_date`をUTC 00:00として扱っており、実際のJST 00:00より9時間遅い時点を起点にしていた(経過時間が実際より9時間短く出る方向。#23が是正した方向と同じ「古いものを新しいと見る」バイアス)。既存の`domain/jst.py::JST`定数を`dt.datetime.combine()`のtzinfoへ使うのみで、新しい変換ロジックは作っていない。新規テスト(`tests/unit/test_issue_578_jpx_staleness_jst.py`)は、この9時間の食い違いが実際に判定を変える境界(45日・90日といった閾値の直前9時間)を固定した。実装中に、`lambda_handlers/watchlist_dispatcher_handler.py::_cache_age_days()`にも同型の欠陥が存在することを確認したが、同ファイルはF-16(D4/D9)であり、D9は#256(JIRO自身が別途保持中)と自己競合するため、本Issueのscopeには含めず**別Issue(#612)へ分割した**(MANAGER判断)。**領域一覧・機能一覧・既存のF行(コメント以外)・共通部品カタログは変更していない。** 判定ロジックの基準日(経過時間の起点)は変更したが、通知内容・保存データ形式は変更していない。実際の判定結果が変わりうるのは、45日/90日の閾値の境界±9時間に該当するキャッシュ取得回のみで、発生頻度は低いと見込んでいる(未実測) |
| 2026-09-26 | F-24(LINE受信と対話応答)へ買付余力(F-54)の参照・棚卸し更新を追加した(Issue #592。#128〔Portfolio Capital Allocation Epic〕A5a。#594/A5cのLINE版follow-up)。新規`ConversationAction.AVAILABLE_CASH_RECONCILE`を追加し、`infrastructure/aws/conversation_state_store.py`の`ConversationState`へ`amount: Decimal | None`フィールドを追加した(既存の`price`フィールドを意味流用しない設計判断。オープン決定事項D2、案(ii)採用)。`record_input()`の`stock_code`引数を`str`必須から`str | None`へ拡張し、SET/REMOVE分岐を他のoptional引数(shares/price/owner)と同型にした(**既存呼び出し元〔BUY/SELL/WATCH〕は引き続き非Noneの値を渡すため挙動は変わらない**。回帰は`test_watch_flow_unaffected_by_optional_stock_code_signature`で固定)。新規`set_available_cash_owner()`(INPUT_WAITING中にownerのみ確定する専用遷移。amount未確定のためCONFIRM_WAITINGへは進まない)を追加した。`services/available_cash_service.py`へ`build_reconcile_plan()`(LINE確認〔confirm〕時点の単発読み取り+`ConditionalPut`構築。既存`reconcile()`〔CLI向け、bounded retry〕とは別経路)を追加した。`infrastructure/aws/conversation_commit.py`へ`commit_available_cash_reconcile()`(`commit_watch()`と同型の最小構成。ConversationStateのclaim消費+対象1件のPut、pauseチェックは不要)を追加した。owner未選択時はQuick Reply(既存owner)に加え自由テキストでの新規owner入力も受け付ける(オープン決定事項D1、USER承認どおり。#589/#594が未登録ownerへの初回reconcileを明示サポートしているため非対称を避けた)。**owner未選択の状態でも`start_or_replace()`でINPUT_WAITINGを作成する設計とした**(1周目設計コメントの「状態はまだ作らない」という記述は、自由テキストowner入力をLINEイベントルーターの既存優先順位〔状態が無いtextはhelp textへ落ちる。変更しない〕を保ったまま成立させるには不整合であったため、実装時に修正した)。lock領域はD5(F-24の実測タグ。1周目設計コメントのD1〔誤り〕から訂正)・D6(F-54)。**領域一覧・機能一覧・既存のF行(F-24/F-54のsource list以外)・共通部品カタログは変更していない。** 既存のBUY/SELL/WATCH/ANALYZE判定・通知内容・保存データ形式(既存collection)・Production挙動の変更なし |
| 2026-09-26 | F-41(インフラ定義・デプロイ)へ、**AvailableCashTable(DynamoDB)の新設とLineWebhookFunctionへのIAM配線**を追加した(Issue #595。#128〔Portfolio Capital Allocation Epic〕A6a。#584/A1・#589/A2・#592/A5aのinfra follow-up)。`infra/template.yaml`へ`AvailableCashTable`(owner単位1レコード、既存`HoldingDecisionRuntimeConfigTable`〔Issue #137〕と同水準の保護: `DeletionPolicy: Retain`・`UpdateReplacePolicy: Retain`・`DeletionProtectionEnabled: true`・PITR有効・`PAY_PER_REQUEST`)を新設した。実際にavailable_cashを読み書きするLambda関数は`LineWebhookFunction`のみ(#592のConversationService経由。CLI〔#594〕は`running_on_lambda()`が常にFalseのためLambda/IAM不要、他の定期実行Lambdaは一切触れない)であることを実装内容から再確認したうえで、IAMを配線した。**当初は既存の共有`DynamoDbCrudAccess` Statement(GetItem/PutItem含む10 action)のResourceへ`AvailableCashTable.Arn`を追加していたが、call graph実測(`AvailableCashService.get()`/`build_reconcile_plan()`・`conversation_commit.commit_available_cash_reconcile()`)により実際に使うのはGetItem・PutItemの2つのみと判明したため、レビュー指摘を受けて専用Statement`AvailableCashAccess`(GetItem+PutItemのみ)へ切り出した**(`TradingPauseConfigConditionCheckAccess`と同じ「共有Statementに混ぜず専用Statementを追加する」既存の流儀を踏襲。DeleteItem/Scan等の未使用actionは含まない)。新規`tests/unit/test_infra_iam_available_cash.py`で、テーブル定義のkey/データ保護設定・`LineWebhookFunction`以外への過剰付与が無いこと・付与actionが`{GetItem, PutItem}`ちょうどであることを検証する(静的YAML解析。実行・AWSアクセスなし)。★ **静的import解析によるLambda→repositoryの到達可能性トレース(`test_infra_iam_v2_tables.py`と同型)は本PRでは追加していない**(#592のPRが本PR時点でmain未マージのため、`conversation_service.py`が`available_cash_service.py`をimportする経路自体がまだmain上に存在せず、その種のテストを今追加すると#592マージまで意味的に自明〔到達不可〕になるか、逆に早すぎる前提を固定してしまうため。#592マージ後に追加することを妨げない)。**ChangeSet CREATE/EXECUTEはいずれも別Human Gate。本PRのmerge単独ではAWSリソースは一切作成・変更されない**(template定義の追加のみ)。lock領域はD9/全領域(F-41の宣言タグどおり)。**領域一覧・機能一覧・既存のF行(F-41のsource list以外)・共通部品カタログは変更していない。** 既存の判定・通知内容・保存データ形式・Production挙動の変更なし(実際のdeployを行うまでAWS上の状態は無変更) |
| 2026-09-26 | F-24(LINE受信と対話応答)・F-26(保有・取得ロット管理)・F-27(取引履歴の取り込み)・F-54(owner単位の買付余力)を跨いで、**通常のLINE会話型「買った」「売った」の確定コミットへ買付余力(available cash)の増減を統合した**(Issue #590。#128〔Portfolio Capital Allocation Epic〕A3-LINE。#584/A1・#589/A2のfollow-up。CLI経路は完了時期がずれるため新規Issue #619〔A3-CLI〕へ分割済み)。`services/available_cash_service.py`へ`build_trade_update_plan(raw_owner, delta, now)`(BUYは負値・SELLは正値のdeltaを現在値へ加算する単発読み取り。`build_reconcile_plan()`と同型)を追加し、新規`AvailableCashNotRegisteredError`(未登録ownerへのTRADE_UPDATEを明示的に拒否する。#584/#589の「未登録≠0円」契約と整合させ、自動0円初期化はしない)を追加した。残高不足(結果がavailable_cash<0)はentity側の既存validatorがplan構築フェーズ(I/O前)でValidationErrorを送出するため、Holding/Transaction/Cashのいずれも書き込まれない(#591の余力不足guardが依拠する契約。`model_copy(update=...)`はPydanticの検証を行わないため使わず、既存constructor経由で新レコードを構築することを確認したうえで実装した)。last_reconciled_atは既存値をそのまま引き継ぎ、TRADE_UPDATEで進めない契約(#584 entity docstring)をここで初めて強制する。`infrastructure/aws/conversation_commit.py`の`commit_buy()`/`commit_sell()`へ`available_cash_put: ConditionalPut`引数を追加し、既存のTransactWriteItems(TradingPauseConfig ConditionCheck・ConversationState消費・Transaction Put・PurchaseLot/Holding Put/Delete)へAvailableCashのConditionalPutを1item追加した(既存API契約を破壊的に変更。呼び出し元は`services/conversation_service.py`の2箇所のみで、いずれも更新済み)。`services/conversation_service.py`の`_commit_buy()`/`_commit_sell()`で`AvailableCashNotRegisteredError`を捕捉し、業務エラーメッセージへ翻訳する層を追加した。1周目のPhase A設計コメント(#590本文)にあった「DOMAIN_LOCK = D1(BUY)/D2(SELL)/D6」は誤りだったため訂正する(`functional_domains.md`のD1は買い候補の探索と買い判定〔F-01/F-02等〕であり、売買**登録**とは無関係の別領域。実際に触れたのはF-24〔D5〕・F-26/F-27/F-54〔D6〕)。lock領域はD5+D6(SHARED `services/write_plan.py`・`infrastructure/aws/dynamodb_transaction.py`は既存の`ConditionalPut`/`conditional_put_transact_item()`を呼ぶのみで無変更。両moduleの領域タグは本PR時点でも「要判断」のまま未解決)。**領域一覧・機能一覧・既存のF行(F-24/F-26/F-27/F-54のsource list以外)・共通部品カタログは変更していない。** 買い候補判定・保有判断スコア・売却判定には影響しない。既存の判定・通知内容・保存データ形式(既存collection)は、買付余力の増減が新たに伴う点を除き変更していない |
| 2026-09-26 | F-16(分散実行)の`lambda_handlers/watchlist_dispatcher_handler.py::_cache_age_days()`で、**JPX候補ユニバースのstaleness監査基準日をUTC暦日からJST暦日へ統一した**(Issue #612。#66 16C。#578の分割先)。`_universe_observation()`が記録する`universe_cache_age_days`/`universe_jpx400_cache_age_days`の算出が、JST公表日である`source_date`をUTC 00:00として扱っており、gate側(`JpxCandidateUniverseProvider._check_staleness()`。#578で是正済み)とinterim windowの間、基準がずれていた(サブちゃんレビュー対応F1'-aが検出: gateはstale判定済みなのに監査は上限未達に見える不整合)。既存の`domain/jst.py::JST`定数を`dt.datetime.combine()`のtzinfoへ使うのみで、新しい変換ロジックは作っていない(#578と同じ変換)。既存テスト`tests/unit/test_issue_69_u1_jpx_vintage_gap.py`(6箇所の期待値をJST基準へ更新。「公表当日はcache_age_daysが負になりうる既知の制約」テストは今回の修正で解消されたため置き換えた)・`tests/unit/test_issue_223_universe_staleness_observability.py`(gate/audit不一致の検出テストを、一致を確認するテストへ置き換え)を更新した。lock領域はD4/D9(F-16の宣言タグ全体)。**領域一覧・機能一覧・既存のF行(コメント以外)・共通部品カタログは変更していない。** 監査記録の経過時間表示のみが変更され、判定ロジック(既存のstaleness gate自体は#578で完了済み・本Issueでは変更していない)・通知内容・保存データ形式(既存フィールドの値のみ)は変更していない |
| 2026-09-26 | F-54(owner単位の買付余力)へ、**BUY登録時のAvailable Cash不足検知**を追加した(Issue #591。#128〔Portfolio Capital Allocation Epic〕A4。#590/A3-LINEのfollow-up)。`services/available_cash_service.py`へ新規`InsufficientAvailableCashError`(BUYの購入金額がavailable_cashを超える場合の専用例外。entity側の汎用`ValidationError`と型で区別できるようにする)を追加し、`build_trade_update_plan()`内で明示的に`new_amount < 0`(delta適用後)を判定してこの例外を送出するよう変更した(entity側の既存validatorはdefense-in-depthとして残す。この専用チェックが機能している限り理論上到達しない)。SELLは常にdelta>=0のためこの検査の対象外(#591 Scopeどおり)。`services/conversation_service.py`の`_commit_buy()`が捕捉する例外を、#590実装時点の暫定対応(`pydantic.ValidationError`の広い捕捉)から`InsufficientAvailableCashError`優先へ切り替えた(`except ValidationError`は同じくdefense-in-depthとして残す)。#590の設計(plan構築をI/O前に完結させる規約)により、拒否時にHolding/Transaction/Cashのいずれも書き込まれないことは追加のtransaction制御なしで自動的に満たされる。lock領域はD6(F-54、主)・D5(F-24、conversation_service.pyの例外捕捉切替のみ)。**領域一覧・機能一覧・既存のF行(F-24/F-54のsource list以外)・共通部品カタログは変更していない。** 買い候補判定・保有判断スコア・売却判定には影響しない。既存の判定・通知内容・保存データ形式(既存collection)は、BUY登録時に余力不足の拒否が新たに発生しうる点を除き変更していない |
| 2026-09-26 | Issue #596(#128 A6b。買付余力機能のProduction初期化・rollout)として、`docs/operations_manual.md`33節へ新設節を追加した(コード変更を伴わない、docsのみ)。A1〜A6a(#584/#589/#590/#591/#592/#593/#594/#595)がすべて`main`へmerge済みだが未deployであることを整理し、初期化戦略(既存owner自動0円backfillはしない)・デプロイ順序(全PRが既にmain上のため、初回deployは1回のChangeSetで行う。段階deployは不要)・rollback方針・Production verification観点をまとめた。**領域一覧・機能一覧・既存のF行・共通部品カタログは変更していない。** 既存の判定・通知内容・保存データ形式・Production挙動の変更なし(実際のChangeSet CREATE/EXECUTEはいずれも別Human Gateであり、本節の追加自体はAWS上の状態を一切変更しない) |
| 2026-09-26 | F-26(保有・取得ロット管理)へ、**CLI経由の通常売買登録(BUY/SELL)をHolding/Transaction/AvailableCashと整合した1更新単位として扱う統合コマンド**を追加した(Issue #619。#128〔Portfolio Capital Allocation Epic〕A3-CLI。#590/A3-LINEから分割したCLI経路)。新規`services/trade_registration_service.py`(`TradeRegistrationService`)へ`register_buy()`/`register_sell()`を追加し、`cli/transactions.py`へ新規コマンド`register-buy`/`register-sell`を追加した(既存の`buy-executed`/`sell-executed`〔Transactionのみ記録〕・`holdings add`〔Holding/Lotのみ記録〕は無変更、独立した経路として併存)。LINE経路(#590/#591)はDynamoDB TransactWriteItemsで原子性を保証するが、CLIは常に`running_on_lambda()`がFalseのローカル実行のみ(Lambda呼び出し元は存在しない)であるため、`PortfolioService._apply_holding_replacement_locally()`と同型の「適用前スナップショット→例外時ロールバック」パターンを、Transaction/Lot/Holding/AvailableCashの4対象へ拡張して実装した。★ **#590 Phase A設計コメントが想定していたLambda向けTransactWriteItems経路(`infrastructure/aws/trade_registration_commit.py`)は、実際の呼び出し元が存在しないためYAGNIとして実装しなかった**(将来Lambda呼び出し元が必要になった場合に追加することを妨げない)。idempotency(D2)は`--idempotency-key`省略時は非冪等(毎回新規uuid4)、指定時は`TransactionRepository.save_if_absent()`(Issue #61 Phase B3の既存プリミティブ)を一意性の権威とする。BUY側のPurchaseLotは`idempotency_key`をそのまま`lot_id`として渡すことで、CSV importが確立した「同一lot_idの再適用は安全」契約(Issue #61 Phase B1)にそのまま乗る(新しい冪等機構は作っていない)。**既存の冪等キー(CSV importの`csv:{sha256}:{row}`等)はすべて内容由来で取り違えが構造的に起こらないが、本Issueで初めて「人が手で打つ自由入力のキー」を導入したため、同一キーを別の取引(所有者・銘柄・株数・単価・約定日・BUY/SELL区分のいずれか)へ誤って使い回すと、その取引が黙って消える(`already_registered=True`が返るだけで実際には何も登録されない)欠陥があった(サブちゃんレビュー#624 F1・F1'・F6で検出、実測で再現確認)。新規`IdempotencyKeyReusedForDifferentTradeError`を追加し、`save_if_absent()`が成功する前(fast-path)・race(他プロセスが先に登録済みだった場合)いずれの経路でも既存Transactionと要求内容を照合し、不一致なら明示的に拒否するよう修正した。約定日を照合対象に含める判断(F1')は、「1回目が実は成功していたのに翌日`--date`省略で再実行すると拒否され得る」というtrade-offより、「exit 0で登録済みと報告したまま取引が黙って消える」実害を優先したfail-closed判断である。**所有者(`owner`)も照合対象へ追加した(サブちゃんレビュー#624 F6。Transactionはowner-scopeでCLIにも`--owner`があり、所有者Aの1回目登録と所有者Bの2回目登録がF1と同じ失敗モードで黙って消えうるため、F1と同じfail-closed方針を適用した)。**あわせて、`register_buy()`が受け取る`account_type`(既定GENERAL)を`build_purchase_write_plan()`〔Lot/Holding〕へは渡していたが`build_execution_plan()`〔Transaction〕へは渡しておらず、Transaction側だけaccount_type=Noneになっていた不整合を修正した(サブちゃんレビュー#624 F7。USER判断: 決定A。サービスAPIとしてaccount_typeを公開している以上、同一取引から生成される永続データ間で一致させるべきという理由)。あわせて、口座種別だけが異なる場合も別取引として扱い、idempotency照合対象へaccount_type(SELLは概念が無いため常にNone)を追加した。★ **サブちゃんレビュー#624 F8対応(テストのみ・実装変更なし)**: F7でaccount_type条件を追加した副作用として、F5(N3)で一度固定したBUY/SELL区分照合の単独性が退行していた(既存テストはBUY既定account_type=GENERALとSELLの常にNoneが異なるため、区分照合だけを外す変異でも別条件〔account_type不一致〕で拒否され通ってしまっていた=SURVIVED)。register_buy()/register_sell()の公開APIだけではaccount_typeを完全に揃えたBUY/SELLの組を作れない(SELLは常にNoneを要求するが、BUY側にNoneを渡すとPurchaseLot/Holdingの必須フィールド検証で失敗するため)ため、既存Transaction側を`TransactionHistoryService`経由で直接account_type=None(SELLが要求する値と同一)で用意し、他6条件を全て揃えたうえで区分だけを変えるテストを追加し、区分照合単独のmutation検出を復元した。★ **サブちゃんレビュー#624 F9対応(テストのみ・実装変更なし)**: SELLの真の冪等retry(同一owner・銘柄・株数・単価・約定日・区分)が正しく`already_registered=True`となることを固定するテストが無く、`register_sell()`のfast-path/race-pathがaccount_type=Noneを渡すべきところを誤って変更する退行が入っても検出できなかったため、専用テストを追加した。両者ともmutationで検出漏れ・検出成立を実測確認し、既存7条件(owner/stock_code/shares/price/execution_date/buy_sell/account_type)全件の変異を再実行して他条件の検出も退行していないことを確認した。★ **サブちゃん再レビュー指摘対応(テストのみ・実装変更なし)**: F9の初回対応はfast-path(`_fast_path_if_already_registered()`)のみを固定しており、race経路(`_commit_locally()`内、`save_if_absent()`がFalseを返す分岐)自身でBUY/SELLそれぞれの真の冪等retryが正しく`already_registered=True`となることは固定されていなかった(register_buy()/register_sell()が`_commit_locally()`へ渡すaccount_typeを誤って固定値へ変更する変異が、BUY側・SELL側とも実測でSURVIVEDすることを確認)。fast-pathを迂回させるテストをBUY/SELL双方に追加し、race経路自身の成功側契約(内容が真に同一なら適用せずno-opで`already_registered=True`を返す)を固定した。いずれもmutationで検出漏れ・検出成立を実測確認済み。**未登録owner・余力不足の拒否は`AvailableCashService`(#591)の既存例外(`AvailableCashNotRegisteredError`/`InsufficientAvailableCashError`)をそのまま再利用する(CLI/LINE間で同じguardを共有)。`infrastructure/local_repository/transaction_repository.py`へ`delete()`(ロールバック専用)を追加した。lock領域はD6(F-26主。F-27/F-54は呼び出しのみで無変更)。**領域一覧・機能一覧・既存のF行(F-26のsource list以外)・共通部品カタログは変更していない。** 買い候補判定・保有判断スコア・売却判定には影響しない。既存の`buy-executed`/`sell-executed`/`holdings add`/LINE経路の挙動・保存データ形式は変更していない |
