...
★ 確認の手順は 3.14節の TARGET_FRESHNESS_CHECK が正本である。
```
...
ACTIVATION_STATE_SSOT = Issue #355 の最新の durable な activation 記録
...
**MANAGER 提案(★ USER 判断待ち。案 (i))**

上記 4 の「取得していないか」の範囲について、REVIEWER から
「他の対象のレビューを通じて、その対象への**言及**を読むことは避けられない」
という指摘がある。範囲を決めないと後から判定できない。

```
案 (i)  対象 = 3.9節 BLIND_FIRST_PHASE_1_FORBIDDEN に列挙された種類を
        ★ 当該対象について取得していないこと
        ★ 他の対象の記録に含まれる当該対象への ★ 言及だけでは
          「取得済み」に当たらない

PRIOR_EXPOSURE  任意の field(3.14節の snapshot)
                Phase 1 の開始時に、その対象について既に目にした他者の結論を
                自己申告する。隠さず、監査できるようにするためである
```

★ この 2 つは **MANAGER の提案であり、USER の判断を待っている**。
★ 否であれば削除すればよい形にしてある(他の規定はこの 2 つに依存していない)。
