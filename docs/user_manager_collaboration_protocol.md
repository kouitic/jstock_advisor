...
```
RECEIPT_EXISTS                   **有効な** RECEIPT が存在する。RECEIPT_STATE が APPROVAL_DECISION と一致しない RECEIPT
                                 (APPROVE 以外なのに APPROVED 等)は無効であり、存在するものとして数えない
...
```
本表の TTL は「標準」である。標準より長い VALID_UNTIL を APPROVAL_REQUEST に書いてよいか(書いてよい場合の条件と、
それを決める者)は、v3 に定めが無い(未決定)。上と同様に、発効の前に USER が決定する必要があり、
本節は AI が推測で補わない。
```
```
...
・request が存在する
・receipt が存在する
・receipt の RECEIPT_STATE が APPROVAL_DECISION と一致する(APPROVE のみ APPROVED。食い違えば無効。
  ai_operation_message_contract.md 8.6.2節)
・REQUEST_ID が一致する
・GATE_TYPE が一致する
・SCOPE が一致する
