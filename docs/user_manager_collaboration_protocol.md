...
役割が別でも、★ その対象について 3.9節の `BLIND_FIRST_PHASE_1_FORBIDDEN` に
該当する情報を先に取得していれば blind-first の独立性は成立しない。
**独立性は session の新しさではなく、レビュー対象ごとに何を先に読んだかで決まる**
(3.9節 / 3.14節)。

**★ 判定は情報の送信者ではなく ★ 内容で行う。**
管理者・利用者・他のレビュー対象の記録から得た情報であることだけを理由に
freshness を失わせない。★ その内容が開発者の完了報告・自己評価・原因説明・
テスト結果の解釈・結論などを実質的に転記・要約したものであれば、
送信者が誰であっても `BLIND_FIRST_PHASE_1_FORBIDDEN` として扱う
(範囲の正本は 3.14節の `TARGET_FRESHNESS_SCOPE`)。
...
★ 新しい session でも、その対象について BLIND_FIRST_PHASE_1_FORBIDDEN に
  該当する情報を先に取得していれば成立しない。
...
TARGET_FRESHNESS_CHECK
  ...
  4  REVIEWER が、その対象について ★ 3.9節の BLIND_FIRST_PHASE_1_FORBIDDEN に
     分類される情報を ★ Phase 1 の snapshot 固定より前に
     既に取得していないかを確認する
     ★ 判定は ★ 「誰から来た情報か」ではなく ★ 「その内容が禁止情報か」で行う
       (下記 TARGET_FRESHNESS_SCOPE)
...
★ ★ 判定は ★ 送信者ではなく ★ 内容で行う。
  MANAGER / USER / 他の対象の記録から得た情報も、その内容が
    developer completion report / developer self-assessment /
    root-cause explanation / test interpretation /
    PASS・問題なし等の developer の結論 / implementation summary
  等を ★ 実質的に転記・要約したものである場合は
  ★ BLIND_FIRST_PHASE_1_FORBIDDEN として扱う。
★ 逆に ★ MANAGER から得た情報であることだけを理由に ★ freshness を失わせない。
```

```
判定の例(★ 網羅ではない。★ 個別の Issue 番号は書かない)

  失わない  当該 Issue の review が停止中であること / 当該 Issue の Priority /
            weekly batch へ載せる予定であること /
            USER が review の再開を承認したこと /
            MANAGER が次の review 対象としていること

  失う候補  developer が実装完了と報告したこと / 問題なしと自己評価したこと /
            root cause が X で修正 Y により解消したという説明 /
            テスト結果から PASS と判断できるという解釈 /
            ★ 上記を MANAGER が実質的に要約・転記したもの
```
