...
_HID_1000 = build_holding_id(DEFAULT_OWNER, "1000")


def test_new_purchase_detected_as_buy() -> None:
    events = detect_trade_events({}, {_HID_1000: _holding("1000", 100)}, _TODAY)
    assert len(events) == 1
    assert events[0].event_type == TransactionType.BUY
    assert events[0].stock_code == "1000"
    assert events[0].holding_id == _HID_1000
    assert events[0].shares == 100


def test_increase_detected_as_additional_buy() -> None:
    previous = {_HID_1000: _snapshot("1000", 100)}
    events = detect_trade_events(previous, {_HID_1000: _holding("1000", 200)}, _TODAY)
    assert len(events) == 1
...
def test_decrease_detected_as_partial_sell() -> None:
    previous = {_HID_1000: _snapshot("1000", 100)}
    events = detect_trade_events(previous, {_HID_1000: _holding("1000", 40)}, _TODAY)
    assert len(events) == 1
...
def test_no_change_produces_no_event() -> None:
    previous = {_HID_1000: _snapshot("1000", 100)}
    events = detect_trade_events(previous, {_HID_1000: _holding("1000", 100)}, _TODAY)
    assert events == []
...
def test_repurchase_after_tombstone_detected_as_buy() -> None:
    ...
    previous = {_HID_1000: _snapshot("1000", 0, active=False)}
    events = detect_trade_events(previous, {_HID_1000: _holding("1000", 50)}, _TODAY)
    assert len(events) == 1
    assert events[0].event_type == TransactionType.BUY
