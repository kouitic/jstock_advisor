...
def test_process_single_candidate_data_not_found_records_data_insufficient(
    ...
    assert audit_calls[0][1] is None
    assert audit_calls[0][2] == "DATA_INSUFFICIENT"
...
def test_process_single_candidate_passed_includes_ranking_entry(
    ...
    monkeypatch.setattr(handler_module, "record_candidate_audit", lambda *a: None)
    record_calls: list[dict[str, Any]] = []
    monkeypatch.setattr(
        handler_module,
        "record_result",
